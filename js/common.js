...
function init_child_menu (id, menus, active) {
    var str = '';
    if (menus) {
        str += '<ul>'
        for (var j = 0; j < menus.length; j++) {
            var item = menus[j]
            var mmmDomId = 'indexMenu' + item.id
            if (item.icon) {
                str += '<li><a id="' + mmmDomId + '" class="hasIcon ' + item.icon + '" data-src="' +
                    item.url + '" data-name="' + item.name + '"><text>'
                    + item.name + '</text>'
            } else {
                str += '<li><a id="' + mmmDomId + '"  data-src="' +
                    item.url + '" data-name="' + item.name + '"><text>'
                    + item.name + '</text>'
            ...
            if (item.children) {
                ...
                for (var k = 0; k < item.children.length; k++) {
                    var item_child = item.children[k]
                    var mmmDomId2 = 'indexMenu' + item_child.id
                    str +=
                        '<li><a id="' +
                        mmmDomId2 +
